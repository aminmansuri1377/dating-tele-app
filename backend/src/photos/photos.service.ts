...
export class PhotosService {
  ...
  constructor(private config: ConfigService, private prisma: PrismaService) {
    this.s3 = new S3Client({
      region: 'auto',
      endpoint: this.config.get<string>('S3_ENDPOINT'), // R2 endpoint, or omit for AWS
      credentials: {
  ...
  async createUploadUrl(userId: string, contentType: string) {
    ...
    return { uploadUrl, storageKey: key, publicUrl: `${this.publicBaseUrl}/${key}` };
  }
  ...
  async confirmUpload(userId: string, storageKey: string, publicUrl: string) {
    ...
    return this.prisma.photo.create({
      data: {
        ...
        storageKey,
        position: count,
        isApproved: true, // set false + route to moderation queue when AI moderation is wired in
      },
    });
  }
